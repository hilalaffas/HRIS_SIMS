...
// [BARU] `editForm`: kalau diisi (elemen React <LeaveForm .../>), modal ini
// beralih ke MODE EDIT -- body diganti jadi form edit tsb & footer lama
// (Edit Berkas/Tutup Detail) disembunyikan, karena LeaveForm sudah punya
// tombol "Simpan Perbaikan"/"Batal Edit" sendiri. Dipakai HANYA oleh
// ApplyCuti.jsx (pemohon). ApproveLeave.jsx & Karyawan.jsx tidak pernah
// mengirim prop ini, jadi tampilan di kedua halaman itu tidak berubah sama
// sekali -- tetap read-only seperti sebelumnya.
const FormCuti = ({ data, onClose, onEdit = null, editForm = null }) => {
  useEffect(() => {
  ...
  const isEditMode = Boolean(editForm);
    ...
    <div className="form-cuti__overlay" onMouseDown={onClose}>
      <section className="form-cuti__modal" role="dialog" aria-modal="true" aria-labelledby="form-cuti-title" onMouseDown={(event) => event.stopPropagation()}>
        <header className="form-cuti__header">
          <div>
            <h2 id="form-cuti-title" className="form-cuti__title">{isEditMode ? 'Edit Berkas Cuti' : 'Informasi Detail Berkas Cuti'}</h2>
            <p className="form-cuti__subtitle">{isEditMode ? 'Perbaiki data pengajuan, lalu kirim ulang untuk diverifikasi.' : 'Pantau alur verifikasi berkas secara berjenjang'}</p>
          </div>
          <button type="button" className="form-cuti__close" onClick={onClose} aria-label="Tutup detail">&times;</button>
        </header>

        {isEditMode ? (
          <div className="form-cuti__body form-cuti__body--edit">
            {editForm}
          </div>
        ) : (
          <>
            ...
            <footer className="form-cuti__footer">
              {/* [UBAH] Sebelumnya onClick memanggil onEdit() LALU onClose()
                  (modal langsung tertutup & pindah ke form di atas halaman).
                  Sekarang cukup panggil onEdit() saja -- ApplyCuti.jsx yang
                  akan mengaktifkan mode edit DI DALAM modal ini juga (lihat
                  handleEditInModal), modal tidak tertutup. */}
              {onEdit && <button type="button" className="form-cuti__edit-btn" onClick={onEdit}>Edit Berkas</button>}
